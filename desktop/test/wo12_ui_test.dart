...
void main() {
  ...
  testWidgets('TxnCard shows REVERSAL badge for refund transactions', (
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: Scaffold(
          body: TxnCard(
            txn: _txn(direction: 'in', reversesTransactionId: 't_original'),
            selected: false,
            onTap: () {},
          ),
        ),
  ...
  testWidgets(
    'TxnCard does not show status badges for evidenced transactions',
    (tester) async {
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
        ...
        home: Scaffold(
          body: SingleChildScrollView(
            child: EvidencePanel(
              card: _card(
                txn,
                evidence: [_ev('d1', 'refund.pdf', role: 'refund_note')],
              ),
            ),
          ),
        ),
  ...
  testWidgets(
    'EvidencePanel unlink calls API, hides evidence, and triggers refresh',
    (tester) async {
      final api = _FakeApi();
      var refreshed = false;
      final txn = _txn(evidence: [_ev('d1', 'invoice.pdf')]);
  ...
  testWidgets(
    'LearningPanel shows Link/Don\'t link/Later for reconciliation questions',
    (tester) async {
      final questions = [
        LearningPrompt(
          id: '1',
  ...
  testWidgets(
    'LearningPanel shows generic buttons for non-reconciliation questions',
    (tester) async {
      final questions = [
        LearningPrompt(
          id: '2',
